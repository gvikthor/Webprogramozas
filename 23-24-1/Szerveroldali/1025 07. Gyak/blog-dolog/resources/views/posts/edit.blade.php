<x-guest-layout>
    <x-slot name="title">Szerk bejegyzés</x-slot>
    ...
    <h1>Szerk bejegyzés</h1>

    <form class="flex flex-col gap-4" action="{{ route('posts.store') }}" method="POST" enctype="multipart/form-data">
        @csrf
        <label for="title">Cím</label>
        <input type="text" name="title" id="title" class="thor-input-field" value="{{ old('title', $post->title) }}">
        @error('title')
            <div class="text-red-500">Cím hiba: {{ $message }}</div>
        @enderror

        <label for="desc">Leírás</label>
        <textarea name="desc" id="desc" cols="30" rows="10" class="thor-input-field">{{ old('desc', $post->desc) }}</textarea>
        @error('desc')
